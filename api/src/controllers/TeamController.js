...
module.exports = {
  CriarTime: async (req, res) => {
    const { nome, member_ids, modalidade_id } = req.body;

    if (!nome) {
      return res.status(400).json({ erro: "Nome do time é obrigatório." });
    }
    if (!Array.isArray(member_ids) || member_ids.length === 0) {
      return res.status(400).json({ erro: "O time deve ter pelo menos 1 membro." });
    }

    console.log(modalidade_id);
    const time = await prisma.times.create({
      data: { nome, modalidade_id },
    });
  ...
  DeletarTime: async (req, res) => {
    ...
    const time = await prisma.times.findUnique({ where: { id } });
    if (!time) {
      return res.status(404).json({ erro: "Time não encontrado." });
    }
