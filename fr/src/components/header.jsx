...
const Header = () => {
  ...
  function AbrirFecharMenu() {
    ...
    if (menu.nextSibling.classList.contains("menu")) {
      classToggle(menu.nextSibling, "flex", "hidden");
    }
  }

  return (
    <>
      <header className="fixed z-30 w-full backdrop-opacity-40 bg-secundaria border-b border-secundaria/50 shadow-md p-2">
        <div className="flex items-center justify-between px-10 py-2">
          ...
          {isLoggedIn ? (
            <>
              ...
              <div className="menu hidden bg-white h-15 w-15 rounded-xl absolute right-11 top-14 flex-col justify-center items-center">
                <Link
                  to="/editar-perfil"
                ...
                <button
                  className="block bg-black text-white w-15 h-[50%] font-semibold text-sl rounded-lg shadow cursor-pointer hover:outline-blue-700 hover:outline-1 hover:bg-neutra-preta transition"
          ...
          <button
            ref={burgerRef}
            onClick={handleClick}
            className="z-50 hidden flex-col justify-center items-center cursor-pointer"
          >
            <span
      ...
      <aside
        ref={menuRef}
        className="hidden z-40 top-16 right-0 h-full w-55 bg-white shadow-lg transform translate-x-full transition-transform duration-300 ease-in-out"
      >
        <div className="flex flex-col items-start p-6 space-y-6 text-primaria text-xl font-medium">
          ...
          {isLoggedIn ? (
            <>
              ...
              <button
                className="block bg-black text-white w-24 h-12 font-semibold text-xl rounded-lg shadow cursor-pointer hover:outline-blue-700 hover:outline-1 hover:bg-neutra-preta transition"
                onClick={handleLogout}
              >
                Sair
              </button>
            ...
            <Link to="/login" onClick={handleClick}>
              <button className="block bg-black text-white w-24 h-12 font-semibold text-xl rounded-lg shadow cursor-pointer hover:outline-blue-700 hover:outline-1 hover:bg-neutra-preta transition">
                Login
              </button>
            </Link>
          )}
