  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!loginResponse.ok) {
        ...
        throw new Error(
          `Erro ${loginResponse.status}: ${errorData.erro || "Falha na verificação"
          }`
        );
      }
  ...
  return (
    <div className="bg-[url(/src/assets/background.png)] bg-no-repeat bg-cover bg-center h-screen flex flex-col justify-center items-center gap-2">
      <div className="bg-[#f5f5f5] rounded-md w-[70%] h-fit flex flex-row justify-between items-start">
          ...
          >
            ...
            <div className="flex flex-col items-center gap-2 w-full">
              <button
                ...
                style={{
                  backgroundColor: hover ? "#F5F5F5" : "#092843", // hover:bg-neutra-branca / bg-secundaria
                  color: hover ? "#001429" : "white",            // hover:text / text-white
                  border: hover ? "1px solid #092843" : "none", // hover:border-secundaria / sem border normal
                ...
                onMouseLeave={() => setHover(false)} type="submit"
                name="submit"
                value="login"
              >
                Verificar
              ...
              <div className="flex flex-col items-center justify-center w-full">
                <span className="text-primaria text-sm text-center w-full cursor-pointer">
                  Reenviar Email
                </span>
              </div>
            </div>
        ...
        <img
          src={sideImage}
          className="block w-[50%] h-full rounded-r-md"
        />
      </div>
    </div>
  );
